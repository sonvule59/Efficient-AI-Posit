...
/* Proper 8-bit Posit Implementation (es=1) following the standard
 * 
 * Based on the posit standard:
 * - 1 sign bit
 * - Variable regime bits (terminated by opposite bit)
 * - 1 exponent bit (es=1)
 * - Remaining fraction bits
 * 
 * For 8-bit posits with es=1:
 * - 2.5 should encode to 0x54 (01010100)
 * - 2.0 should encode to 0x50 (01010000)
 * - 4.5 should encode to 0x5c (01011100)
 */
...
static posit8_error_t decode_posit8(posit8_components *comp, posit8 p) {
    ...
    // Regime - count consecutive bits after sign until opposite bit
    int regime_len = 0;
    int regime_bit = (abs_p >> 6) & 1;
    int i = 0;
    while (i < 7 && ((abs_p >> (6-i)) & 1) == regime_bit) {
        regime_len++;
        i++;
    }
    // Regime value is (regime_len - 1) if regime_bit=1, else -(regime_len - 1)
    comp->regime = regime_bit ? (regime_len - 1) : -(regime_len - 1);

    // Exponent (1 bit, es=1)
    int exp_offset = regime_len;
    if (exp_offset < 7) {
        comp->exponent = (abs_p >> (6 - exp_offset)) & 1;
    } else {
        comp->exponent = 0;
    }

    // Fraction (remaining bits)
    comp->frac_bits = 7 - regime_len - 1; // 7 total bits minus regime and exp
    if (comp->frac_bits > 0) {
...
/* Encode components to posit8 with proper rounding */
static posit8_error_t encode_posit8(posit8 *result, posit8_components comp) {
    ...
    if (comp.regime >= 0) {
        ...
        if (regime_ones > 6) regime_ones = 6;
        bits |= ((1 << regime_ones) - 1) << (6 - regime_ones);
    } else {
    ...
    // Exponent bit
    int regime_bits = abs(comp.regime) + 1;
    int exp_pos = 6 - regime_bits;
    if (exp_pos >= 0) {
...
/* Convert double to posit8 with proper rounding */
posit8_error_t posit8_from_double(posit8 *result, double d) {
    ...
    // Calculate fraction
    double frac = tmp - 1.0;
    comp.frac_bits = 7 - (abs(k) + 1) - 1; // 7 total bits minus regime and exp
    if (comp.frac_bits > 0) {
